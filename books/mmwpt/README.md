...
Receiving grants
- Authorization grant
    - Consists of an authorization link.
    - `https://www.example.com/oauth/authorize?response_type=code&client_
id=CLIENT_ID&redirect_uri=CALLBACK_URL&scope=read`
    - Once the flow finishes the redirect URL is fetched with `code` param which contains the grant code generated by the auth server.
